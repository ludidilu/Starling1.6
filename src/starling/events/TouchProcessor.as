...
package starling.events
{
    ...
    public class TouchProcessor
    {
		...
		private var touchTargetDic:Dictionary;//陆地添加的  为了防止一个touch事件在一帧内触发多次
                                          ...
                                          shiftDown:Boolean, ctrlDown:Boolean):void
        {
            ...
            for each (touch in touches){
				...
				if(touch.target){
					...
					if(touchTargetDic[touch.target]){//陆地添加的  为了防止一个touch事件在一帧内触发多次
						
						if(touchTargetDic[touch.target][touch.phase]){
...
//							trace("本来会报错的  现在保护掉了！！！");
							...
							touchTargetDic[touch.target][touch.phase] = true;
						}
						
					}else{
						
						touchTargetDic[touch.target] = new Dictionary;
